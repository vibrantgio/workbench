...
// showing under it the style's dominant colours and the primary pair its
// leading one derives, in name order, and only the ones fitted to the
// appearance on screen. One click takes both halves of a theme off a card: the
// seed from its leading colour, and the syntax base for each appearance, the
// style itself on the side it was fitted to and the nearest measured
// counterpart on the other. Everything after that click is what it is after a
// drop — the row still offers the style's other colours, the list beside the
// code still overrides either member, and keeping still writes the lot.
//
