...
// Code generated from golang.org/x/exp/shiny/materialdesign/icons — the
// exported IconVG vars of that package, in source order. Regenerate with:
//
//   grep -o '^var [A-Za-z0-9]*' $(go env GOMODCACHE)/golang.org/x/exp/shiny*/materialdesign/icons/data.go \
//     | awk '{printf "\t{%c%s%c, icons.%s},\n", 34, $2, 34, $2}'
//
// Go offers no way to enumerate a package's vars at runtime, so the pairing
// of name and data is spelled out here once.

import "golang.org/x/exp/shiny/materialdesign/icons"

// NamedIcon pairs an icon's exported name with its IconVG data.
type NamedIcon struct {
	Name string
	Data []byte
}

// IconTable lists every Material Design icon the shiny package publishes.
var IconTable = []NamedIcon{
