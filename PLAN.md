...
- [ ] In the live `Modal` path render the close affordance with `button.Button(...)` (GX.3 icon variant) passing the × `clip.Path` painter and `Props.Clickable: &st.closeClick`; in the static `Render` path use `button.RenderIcon(...)` with the same painter for deterministic goldens.
