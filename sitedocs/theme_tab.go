...
// the themer's palette section (the ramps grid and the named picks, drawn
// by theme_palette.go's copy of that presentation) followed by the
...
func themeTabRows(inv *inventory.Inventory, shaper *text.Shaper, typo tokens.Typography, c tokens.ColorTokens) []layout.Widget {
	rows := themePaletteRows(shaper, typo, c)
